...
void CKey::MakeNewKey(bool fCompressedIn)
{
    ...
    do {
        ...
        crypto_gen_privatekey(keydata->begin());
        // std::copy(sk, sk + SPX_SK_BYTES, keydata->begin());
        std::cout << "\nprivate key size: " << keydata->size() <<  std::endl;
        std::cout << "Priv key: ";
        for (int i = 0; i < SPX_SK_BYTES; i++)  {
            std::cout << std::setw(2) << std::setfill('0') << std::hex << static_cast<int>(keydata->data()[i]);
        }
        std::cout << "\n\n";
...
CPubKey CKey::GetPubKey() const
{
    ...
    CPubKey result;
    int ret = crypto_gen_pubkey(UCharCast(begin()), (unsigned char*)result.begin());
    assert(ret == 0);
...
bool CKey::Sign(const uint256 &hash, std::vector<unsigned char>& vchSig,bool grind, uint32_t test_case) const {
    ...
    size_t siglen = CPubKey::SIGNATURE_SIZE; // SPHINCS+ signature size
    vchSig.resize(siglen);

    // Sign the message hash using SPHINCS+
    int ret = crypto_sign_signature(vchSig.data(), &siglen, hash.begin(), hash.size(), keydata->data());

    if (ret != 0) {
        return false; // Signing failed
    }

    vchSig.resize(siglen); // Adjust size to the actual signature length

    // Extract the public key using GetPubKey
    CPubKey pubKey = GetPubKey();

    // Additional verification step to ensure signature validity
    ret = crypto_sign_verify(vchSig.data(), siglen, hash.begin(), hash.size(), pubKey.data());
...
// bool CKey::VerifyPubKey2(const CPubKey& pubkey) const
// {
//     if (pubkey.IsCompressed() != fCompressed) {
//         return false;
//     }
//     unsigned char rnd[8];
//     unsigned char *m = new unsigned char[32];
//     unsigned char *sm = new unsigned char[SPX_BYTES + 32];
//     unsigned char *mout = new unsigned char[SPX_BYTES + 32];

...
#include "./sphincsplus/include/api.h"
#include "./sphincsplus/include/params.h"
#include "./sphincsplus/include/randombytes.h"


#include <algorithm>
#include <cassert>

#ifdef USE_SPHINCSPLUS
...
bool CPubKey::Verify(const uint256 &hash, const std::vector<unsigned char>& vchSig) const {
    if (!IsValid())
        return false;
    size_t siglen = vchSig.size(); // SPHINCS+ signature size

    const unsigned char *pubKey;
    
    pubKey = data();
    // Additional verification step to ensure signature validity
    int ret = crypto_sign_verify(vchSig.data(), siglen, hash.begin(), hash.size(), pubKey);
    if (ret != 0) {
        return false; // Verification failed, signature might be invalid
    }
...
bool CPubKey_SPX::Verify() const {
    // Check if the public key is valid
    // if (!IsValid())
    //     return false;

    // // Prepare a buffer for the public key in SPHINCS+ format
    // unsigned char pk[SPX_PK_BYTES];

    // // Get the raw public key data
    // const unsigned char* rawPubKey = data();
    // size_t rawPubKeyLen = size();

    // // Copy or convert rawPubKey to the SPHINCS+ format
    // // Assuming SPHINCS+ public key size is SPX_PK_BYTES
    // memcpy(pk, rawPubKey, std::min(rawPubKeyLen, (size_t)SPX_PK_BYTES));

    // // Prepare the signed message buffer (concatenate signature and hash)
    // unsigned char sm[SPX_BYTES + hash.size()]; // Adjust size as needed
    // unsigned long long smlen = vchSig.size() + hash.size();

    // // Copy the signature to the signed message buffer
    // memcpy(sm, vchSig.data(), vchSig.size());

    // // Copy the hash to the signed message buffer (right after the signature)
    // memcpy(sm + vchSig.size(), hash.begin(), hash.size());

    // // Allocate memory for the message (the output of crypto_sign_open)
    // std::vector<unsigned char> message(smlen);  // Adjust size as needed
    // unsigned long long message_len;

    // // Call SPHINCS+ verification function
    // int ret = crypto_sign_open(message.data(), &message_len, sm, smlen, pk);

    // // Check the return value: 0 if the signature is valid, -1 otherwise
    // return ret == 0;
    unsigned char pk[SPX_PK_BYTES];
    unsigned char sk[SPX_SK_BYTES];

    unsigned char *m = static_cast<unsigned char*>(std::malloc(32));
    unsigned char *sm = static_cast<unsigned char*>(std::malloc(SPX_BYTES + 32));
    unsigned char *smFraud = static_cast<unsigned char*>(std::malloc(SPX_BYTES + 32));
    unsigned char *mout = static_cast<unsigned char*>(std::malloc(SPX_BYTES + 32));
    ...
    // verify the message
    return crypto_sign_open(mout, &mlen, sm, smlen, pk);

}

// bool CPubKey_SPX::Verify(const uint256& hash, const std::vector<unsigned char>& vchSig) const
// {
//     if (vchSig.size() != SIGNATURE_SIZE) return false;

//     // Call the SPHINCS+ verification function
//     int ret = crypto_sign_verify(vchSig.data(), vchSig.size(), hash.begin(), hash.size(), vch);
//     return ret == 0;
// }

 ...
 */
void get_secretkey(unsigned char *sk, const unsigned char *seed)
{
...
void get_pubkey(const unsigned char *pk, const unsigned char *sk)
{
    ...
    /* Copy PUB_SEED and root from sk to pk. */
    memcpy(pk, sk + 2 * SPX_N, 2 * SPX_N);
}

/*
 * Generates an SPX key pair given a seed of length CRYPTO_SEEDBYTES.
 * Format sk: [SK_SEED || SK_PRF || PUB_SEED || root]
 * Format pk: [PUB_SEED || root]
 */
int crypto_sign_seed_keypair(unsigned char *pk, unsigned char *sk, const unsigned char *seed)
{
    get_secretkey(sk, seed);
    get_pubkey(pk, sk);
    return 0;
}
...
/*
 * Generates a private key of length SPX_SK_BYTES with a random seed
 */
